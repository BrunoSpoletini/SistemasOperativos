...
nombre( {persona, {nombre, N}, {apellido, A}} ) ->  N.
     
apellido( {persona, {nombre, N}, {apellido, A}} ) ->  A.

tuple_test (P1, P2) ->
    io:fwrite("El nombre de P1 es ~p y el apellido de P2 es ~p~n", [nombre(P1), apellido(P2)]).


filtrar_por_apellido(Personas, Apellido) -> [ N ||  {persona, {nombre, N}, {apellido, A}} <- Personas, A == Apellido].
